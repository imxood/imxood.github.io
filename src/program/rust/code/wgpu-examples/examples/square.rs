...
impl Vertex {
    const VERTEX_ATTRIBUTES: [wgpu::VertexAttribute; 3] = wgpu::vertex_attr_array![
        0 => Float32x2, 1 => Float32x3, 2 => Float32x2,
    ];
...
impl State {
    async fn new(window: &Window) -> Self {
        ...
        */
        ...
        let image_texture = Texture::from_bytes(
            &device,
            &queue,
        ...
        let index_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("Vertex Buffer"),
            contents: bytemuck::cast_slice(INDEIES),
            usage: wgpu::BufferUsages::INDEX,
        });

        let index_number = INDEIES.len();
...
impl Texture {
    pub fn from_raw(
        ...
        let image = image::DynamicImage::ImageLuma8(buf);
